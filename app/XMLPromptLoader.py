    ...
    def load_app_config(self, config_xml_path: str):
        try:
            ...
            if app_config is not None:
                ...
                pp = app_config.find("prompt_config_path").text
                app_config_dict["prompt_config_path"] = pp
    ...
    def load_prompt_config(self, prompt_xml_path: str, lang: str):
        try:
            ...
            if prompt_config is not None:
                ...
                for field in fields:
                    tag = prompt_config.find(field).find(lang)
                    if tag.tag == lang:
                        prompt_config_dict[field] = tag.text.strip()
                    else:
                        prompt_config_dict[field] = f"⚠️ '{field}' no definido para idioma '{lang}'"
